...
function SignupP() {
  ...
  useEffect(() => {
    ...
    const finishGoogleSignup = async () => {
      ...
      const hasGoogleAuthParams = params.has('accessToken') ||
      ...
      if (!hasGoogleAuthParams) return;

      setIsGoogleLoading(true);
      try {
        const data = await completeGoogleLogin(location.search, '/signup');
        if (!data?.user) return;

        setUser(data.user);
        toast.success('Conta conectada com Google!');
        navigate(nextRouteAfterAuth(data.user), { replace: true });
      } catch (err) {
        toast.error(err.message || 'Não foi possível continuar com Google.');
              ...
              <div className={styles.stepContent} key="step2">
                ...
                <div className={styles.btnRow}>
                  ...
                  <button
                    type="submit"
                    className={styles.submitBtn}
                    disabled={isLoading || isGoogleLoading || !allPasswordValid || !passwordsMatch}
                  >
                    {isLoading ? (
